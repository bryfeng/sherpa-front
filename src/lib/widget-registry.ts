...
export const WIDGET_REGISTRY: Record<WidgetKind, WidgetMetadata> = {
  ...
  'polymarket-market-detail': {
    kind: 'polymarket-market-detail',
    category: 'data',
    name: 'Market Detail',
    description: 'Detailed view of a prediction market with prices and AI analysis',
    icon: 'bar-chart-2',
    defaultSize: 'wide',
    minSize: { cols: 4, rows: 2 },
    maxSize: { cols: 12, rows: 4 },
    ...
    requiresWallet: false,
    requiresPro: false,
    tags: ['polymarket', 'prediction', 'detail', 'analysis'],
  },
  'polymarket-portfolio': {
    kind: 'polymarket-portfolio',
    category: 'data',
...
function getDefaultPayload(kind: WidgetKind): WidgetPayload {
  const defaults: Record<WidgetKind, WidgetPayload> = {
    ...
    // Polymarket widgets
    'polymarket-markets': {} as Record<string, unknown>,
    'polymarket-market-detail': { marketId: '' },
    'polymarket-portfolio': { address: '' },
    'polymarket-leaderboard': { sortBy: 'total_pnl' },
  }
