...
import { getRelayQuote } from '../../services/relay'

export function RelayQuoteModal({ onClose }: { onClose: () => void }) {
  ...
  const [userAddress, setUserAddress] = useState('0x50ac5CFcc81BB0872e85255D7079F8a529345D16')
  ...
  async function preview() {
    ...
    try {
      const payload = {
        ...
        amount: amountWei,
        referrer: 'relay.link/poc',
        useExternalLiquidity: false,
        useDepositAddress: false,
        topupGas: false,
      }
      const data = await getRelayQuote(payload)
      setResult(data)
    } catch (e: any) {
  ...
  return (
    ...
    >
      <div className="space-y-3 text-sm">
        ...
        <div>
          <div className="text-slate-700 mb-1">User / recipient address</div>
          <input value={userAddress} onChange={(e) => setUserAddress(e.target.value)} className="w-full rounded-xl border border-slate-300 px-3 py-2" />
        </div>
        <div>
