...
import {
  ...
  toHex,
  validate
} from "./chunk-FW76OZK3.js";
import {
  AccountNotFoundError,
  formatLog,
  formatTransactionReceipt
} from "./chunk-E3BPVJ2E.js";
import {
...
function ecdh(Point3, ecdhOpts = {}) {
  ...
  return Object.freeze({ getPublicKey: getPublicKey4, getSharedSecret, keygen, Point: Point3, utils: utils2, lengths });
}
function ecdsa(Point3, hash9, ecdsaOpts = {}) {
  ahash(hash9);
  ...
  const { keygen, getPublicKey: getPublicKey4, getSharedSecret, utils: utils2, lengths } = ecdh(Point3, ecdsaOpts);
  ...
  function sign8(message, secretKey, opts = {}) {
...
function weierstrass(c) {
  ...
  const Point3 = weierstrassN(CURVE, curveOpts);
  const signs = ecdsa(Point3, hash9, ecdsaOpts);
...
function pointG2FromBytes(bytes) {
  ...
  const slc = (b, from35, to3) => bytesToNumberBE(b.slice(from35, to3));
...
var HDKey = class _HDKey {
  ...
  sign(hash9) {
    ...
    abytes(hash9, 32);
    return secp256k12.sign(hash9, this.privKey).toCompactRawBytes();
  }
  verify(hash9, signature) {
    ...
    return secp256k12.verify(sig, hash9, this.publicKey);
  }
  wipePrivateData() {
    this.privKey = void 0;
...
var noble3 = secp256k12;
function getPublicKey3(options) {
  const { privateKey } = options;
  const point = secp256k12.ProjectivePoint.fromPrivateKey(from3(privateKey).slice(2));
  return from4(point);
}
...
function sign5(options) {
  const { extraEntropy: extraEntropy3 = extraEntropy, hash: hash9, payload, privateKey } = options;
  const { r, s, recovery } = secp256k12.sign(from2(payload), from2(privateKey), {
    extraEntropy: typeof extraEntropy3 === "boolean" ? extraEntropy3 : from3(extraEntropy3).slice(2),
    lowS: true,
    ...hash9 ? { prehash: true } : {}
  });
...
function verify5(options) {
  ...
  return secp256k12.verify(signature, from2(payload), toBytes3(publicKey), ...hash9 ? [{ prehash: true, lowS: true }] : []);
}
...
var isJapanese = (wordlist21) => wordlist21[0] === "あいこくしん";
function nfkd(str) {
  if (typeof str !== "string")
...
function generateMnemonic(wordlist21, strength = 128) {
  anumber(strength);
  if (strength % 32 !== 0 || strength > 256)
    throw new TypeError("Invalid entropy");
  return entropyToMnemonic(randomBytes(strength / 8), wordlist21);
}
var calcChecksum = (entropy) => {
  const bitsLeft = 8 - entropy.length / 4;
  return new Uint8Array([sha2562(entropy)[0] >> bitsLeft << bitsLeft]);
};
function getCoder(wordlist21) {
  if (!Array.isArray(wordlist21) || wordlist21.length !== 2048 || typeof wordlist21[0] !== "string")
    throw new Error("Wordlist: expected array of 2048 strings");
  wordlist21.forEach((i) => {
    if (typeof i !== "string")
      throw new Error("wordlist: non-string element: " + i);
  });
  return utils.chain(utils.checksum(1, calcChecksum), utils.radix2(11, true), utils.alphabet(wordlist21));
}
function mnemonicToEntropy(mnemonic, wordlist21) {
  const { words } = normalize2(mnemonic);
  const entropy = getCoder(wordlist21).decode(words);
  aentropy(entropy);
  return entropy;
}
function entropyToMnemonic(entropy, wordlist21) {
  aentropy(entropy);
  const words = getCoder(wordlist21).encode(entropy);
  return words.join(isJapanese(wordlist21) ? "　" : " ");
}
function validateMnemonic(mnemonic, wordlist21) {
  try {
    mnemonicToEntropy(mnemonic, wordlist21);
  } catch (e) {
    return false;
  }
  return true;
...
// node_modules/ox/_esm/core/Mnemonic.js
function random3(wordlist21, options = {}) {
  const { strength = 128 } = options;
  return generateMnemonic(wordlist21, strength);
}
...
function validate9(mnemonic, wordlist21) {
  return validateMnemonic(mnemonic, wordlist21);
}
...
@scure/bip39/esm/index.js:
  (*! scure-bip39 - MIT License (c) 2022 Patricio Palladino, Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=chunk-MSJ2SSLO.js.map
